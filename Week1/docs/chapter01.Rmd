...
case0101 %>%
  group_by(Treatment) %>%
  summarise(min = min(Score), Q1 = quantile(Score, probs = .25),
...
The 32 men have an average starting salary that is \$818 more than the 61 women (\$5957 vs
\$5139). Both distributions have similar spread (sd=\$539.87 for women and sd=\$690.73 for men)
and distributions that are approximately normally distributed (see density plot). The key difference between the groups is the shift (as indicated by the parallel boxplots).
...
```{r fig.height=3, fig.width=5}
permTest(Salary ~ Sex, data = case0102, alternative = "two.sided",
         legend.loc="topleft")
```
...
close to our observed value. Therefore, we reject the null hypothesis (p < 0.001) and conclude that the salaries of the men are higher than that of the women back in the population.